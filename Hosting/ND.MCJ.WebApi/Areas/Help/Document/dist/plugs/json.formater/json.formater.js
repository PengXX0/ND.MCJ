...
JsonFormater.prototype = {
    ...
    doFormat: function (json) {
        ...
        try {
            ...
            this.isFormated = false;
        } this.bindEvent(); return this;
    },
    bindEvent: function () {
        var that = this;
        $(this.options.dom).find("i").parent("span").unbind("click").bind("click", function () {
            if (that.isFormated == false) { return; }
            that.makeContentVisible($(this).next());
        });
    },
    expandAll: function () {
    ...
    makeContentVisible: function (element) {
        element.prev().find("font").text(element.is(":hidden") ? "" : "...");
        element.toggle().prev().find('i')
        .toggleClass(this.options.expanded + " " + this.options.collapsed);
    },
    processObject: function (obj, indent, addComma, isArray, isPropertyContent) {
        ...
        if ($.isArray(obj)) {
            if (obj.length == 0) {
                ...
                clpsHtml = this.options.isCollapsible ? "<span class='jf-collapsWrapper'><i class='" + this.options.expanded + "'/><font></font></span><span class='jf-collapsible'>" : "";
