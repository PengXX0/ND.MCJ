...
using System.Linq;
using System.Text;
namespace GeneratModels
{
    public static class BusinessUtilities
    {
        public static void Run(string[] args)
        {
            var list = GetDbStructures();
            var tables = list.GroupBy(s => s.TableName).OrderBy(s => s.Key);
            ...
            switch (value)
            {
                case 0://生成单文件Model
                    foreach (var item in tables)
                    {
                        ...
                        templateString += ModelTemplate(fieldList);
                    }
                    templateString = FrameTemplate(templateString);
                ...
                case 1://生成多文件Model
                    foreach (var item in tables)
                    {
                        ...
                        var fieldList = list.Where(s => s.TableName == item.Key).ToList();
                        templateString = ModelTemplate(fieldList);
                        GenerateTemplate(FrameTemplate(templateString), item.Key);
                    }
                    break;
                case 2://生成DbSet
                    GenerateTemplate(DbSetModel(tables), "DbSet");
                    break;
            }
        }

        public static String DbSetModel(IEnumerable<IGrouping<String, DbModel>> tables)
        {
            var builder = new StringBuilder();
            foreach (var item in tables)
            {
                builder.AppendFormat("public DbSet<{0}> {0} {{ get; set; }}", item.Key).Append(Environment.NewLine);
            }
...
ORDER BY 
    A.ID,A.COLORDER";
        ...
        public static String FrameTemplate(String modelsString)
        {
            ...
            .AppendFormat("namespace {0}", ConfigurationManager.AppSettings["Namespace"]).Append(Environment.NewLine)
        ...
        public static String ModelTemplate(List<DbModel> list)
        {
            ...
            .Append("    public class {1} {3}").Append(Environment.NewLine)
            .Append("    {{").Append(Environment.NewLine)
            .Append("{2}")
            .Append("    }}").Append(Environment.NewLine);
